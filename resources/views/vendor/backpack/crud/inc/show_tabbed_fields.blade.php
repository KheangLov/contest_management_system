        ...
        <ul class="nav {{ $horizontalTabs ? 'nav-tabs' : 'flex-column nav-pills'}} {{ $horizontalTabs ? '' : 'col-md-3' }}" role="tablist">
            @foreach ($crud->getTabs() as $k => $tab)
                ...
                <li role="presentation" class="nav-item">
                    <a href="#tab_{{ Str::slug($tabs[0]) }}"
                        aria-controls="tab_{{ Str::slug($tabs[0]) }}"
                        role="tab"
                        tab_name="{{ Str::slug($tabs[0]) }}"
                        data-toggle="tab"
                        class="nav-link {{ isset($tabWithError) ? ($tabs[0] == $tabWithError ? 'active' : '') : ($k == 0 ? 'active' : '') }}"
                        >{{ isset($tabs[1]) ? $tabs[1] : '' }}</a>
                </li>
            @endforeach
        </ul>
        ...
        <div class="tab-content p-0 {{$horizontalTabs ? '' : 'col-md-9'}}">
            ...
            @foreach ($crud->getTabs() as $k => $tab)
                @php
                    $tabs = explode(',', $tab);
                @endphp
                <div role="tabpanel" class="tab-pane {{ isset($tabWithError) ? ($tabs[0] == $tabWithError ? ' active' : '') : ($k == 0 ? ' active' : '') }}" id="tab_{{ Str::slug($tabs[0]) }}">
